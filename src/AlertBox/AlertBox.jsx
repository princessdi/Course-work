...
function AlertBox(props) {
    ...
    const [exampleAlerts, setExampleAlerts] = useState([
        {
    ...
    return (
        <div>
            {props.isReady ? <>
                {/* Стихийные бедствия по выбранному месту*/}
                <h1>Alerts</h1>
                {props.alerts.length == 0 ? <h3>Everything is okay</h3> : props.alerts.map((item, index) => {
                    return <div key={`item-${index}`} className={styles.card}>
                        <p className={styles.title}>{item.event}</p>
                        <p>{item.headline}</p>
                        <p>{item.note}</p>
                    </div>
                })}
                ...
                {/* Шаблонные бедствия*/}
                <h1>Example of the alerts</h1>
                {exampleAlerts.map((item, index) => {
                    return <div key={`item-${index}`} className={styles.card}>
                        <p className={styles.title}>{item.event}</p>
                        <p>{item.headline}</p>
                        <p>{item.note}</p>
